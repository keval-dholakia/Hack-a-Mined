...
function KPI({
  ...
  return (
    <div className={joStyles.kpiCard}>
      <div
        className={joStyles.kpiIcon}
        style={{ color, background: `${color}18` }}
      >
        {icon}
      </div>
      <div>
        <p className={joStyles.kpiLabel}>{label}</p>
        <p className={joStyles.kpiValue} style={{ color }}>
          {value}
        </p>
      </div>
    </div>
...
export default function JobOrderAnalytics({ records, onClose }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    // Hours
    const totalEstHrs = records.reduce(
      (s, r) => s + r.operations.reduce((a, o) => a + o.est_hrs, 0),
      0,
    );
    const totalActHrs = records.reduce(
      (s, r) => s + r.operations.reduce((a, o) => a + o.actual_hrs, 0),
      0,
    );
    const efficiency =
      totalEstHrs > 0 ? Math.round((totalActHrs / totalEstHrs) * 100) : 0;
    ...
    const prioPie = Object.entries(prioMap)
    ...
    const monthData = Array.from({ length: 12 }, (_, m) => {
      ...
      return {
        label: lbl,
        total: mos.length,
        completed: mos.filter((r) => r.status === "Completed").length,
      };
    });
    ...
    const wcMap: Record<
      string,
      { total: number; completed: number; estHrs: number; actHrs: number }
    > = {};
    records.forEach((r) => {
      if (!wcMap[r.work_center])
        wcMap[r.work_center] = { total: 0, completed: 0, estHrs: 0, actHrs: 0 };
      wcMap[r.work_center].total++;
      if (r.status === "Completed") wcMap[r.work_center].completed++;
      wcMap[r.work_center].estHrs += r.operations.reduce(
        (a, o) => a + o.est_hrs,
        0,
      );
      wcMap[r.work_center].actHrs += r.operations.reduce(
        (a, o) => a + o.actual_hrs,
        0,
      );
    });
    const wcData = Object.entries(wcMap)
      .sort((a, b) => b[1].total - a[1].total)
      .map(([name, d]) => ({
        name,
        ...d,
        eff: d.estHrs ? Math.round((d.actHrs / d.estHrs) * 100) : 0,
      }));

    // Operator breakdown
    const opMap: Record<
      string,
      { total: number; completed: number; qty: number }
    > = {};
    records.forEach((r) => {
      if (!opMap[r.operator])
        opMap[r.operator] = { total: 0, completed: 0, qty: 0 };
    ...
    records.forEach((r) =>
      r.operations.forEach((o) => {
        opsFreq[o.operation] = (opsFreq[o.operation] ?? 0) + 1;
      }),
    );
    const topOps = Object.entries(opsFreq)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([name, value]) => ({ name, value }));

    const radarData = wcData
      .slice(0, 6)
      .map((w) => ({
        wc: w.name.length > 8 ? w.name.slice(0, 8) + "…" : w.name,
        total: w.total,
    ...
    <div className={joStyles.analyticsPage}>
      <div className={joStyles.analyticsHeader}>
        <div>
          ...
          <p className={joStyles.analyticsSub}>
            360° production data visualization · {stats.total} orders
          </p>
        </div>
        <button className={joStyles.backBtn} onClick={onClose}>
          ← Back to List
        </button>
      </div>
      ...
      <div className={joStyles.kpiStrip}>
        ...
        <KPI
          icon="◇"
          label="Est vs Act Hrs"
          value={`${stats.totalEstHrs}h / ${stats.totalActHrs}h`}
          color={PURPLE}
        />
      </div>
      ...
      <div className={joStyles.tabBar}>
        {tabs.map((t) => (
          <button
            key={t.id}
            className={`${joStyles.aTab} ${tab === t.id ? joStyles.aTabActive : ""}`}
            onClick={() => setTab(t.id)}
          >
            {t.label}
          </button>
          ...
          <div className={joStyles.aCard}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                ...
                >
                  {stats.prioPie.map((e, i) => (
                    <Cell key={i} fill={PRIORITY_COLOR[e.name] ?? COLORS[i]} />
                  ))}
                </Pie>
                <Tooltip content={<DarkTooltip />} />
                <Legend
              ...
              <tbody>
                {stats.wcData.map((w, i) => (
                  <tr key={w.name}>
                    ...
                    <td className={joStyles.mono} style={{ color: GREEN }}>
                      {w.completed}
                    </td>
                    ...
                    <td>
                      <span
                        style={{
                          ...
                          fontSize: "0.72rem",
                          fontWeight: 700,
                          background: w.eff <= 100 ? `${GREEN}22` : `${ROSE}22`,
                          color: w.eff <= 100 ? GREEN : ROSE,
                        }}
                      >
                        {w.eff}%
                      </span>
                      ...
                      <td>
                        <span
                          style={{
                            ...
                            fontWeight: 700,
                            background:
                              op.rate >= 50 ? `${GREEN}22` : `${AMBER}22`,
                            color: op.rate >= 50 ? GREEN : AMBER,
                          }}
                        >
                          {op.rate}%
                        </span>
      ...
      {tab === "trend" && (
        <div className={joStyles.aGrid}>
          <div className={joStyles.aCard} style={{ gridColumn: "span 2" }}>
            <p className={joStyles.aCardTitle}>
              12-Month JO Trend{" "}
              <span className={joStyles.aBadge}>planned · completed</span>
            </p>
            <ResponsiveContainer width="100%" height={280}>
              ...
              <PieChart>
                <Pie
                  data={[
                    ...
                    {
                      name: "Open/Planned",
                      value: stats.planned + stats.draft,
                    },
            ...
            <div className={joStyles.healthPanel}>
              {[
                ...
                },
                {
                  ...
                  val: stats.total
                    ? Math.round(
                        ((stats.onHold + stats.cancelled) / stats.total) * 100,
                      )
                    : 0,
                  color: ROSE,
                },
                ...
                <div key={row.label} className={joStyles.hRow}>
                  ...
                  <div className={joStyles.hTrack}>
                    <div
                      className={joStyles.hFill}
                      style={{ width: `${row.val}%`, background: row.color }}
                    />
                  </div>
                  <span className={joStyles.hPct} style={{ color: row.color }}>
                    {row.val}%
                  </span>
                </div>
              ))}
              <div className={joStyles.insight}>
                <span>
                  {stats.completionRate >= 70
                    ? "Excellent production flow — over 70% JOs completed."
                    : stats.completionRate >= 50
                      ? "Good progress. Monitor in-progress orders for delays."
                      : "Focus on clearing backlog and resolving on-hold orders."}{" "}
                  Total estimated <strong>{stats.totalEstHrs}h</strong>, actual{" "}
                  <strong>{stats.totalActHrs}h</strong> logged across{" "}
                  <strong>{stats.total}</strong> job orders.
                </span>
              </div>
            </div>
          </div>
