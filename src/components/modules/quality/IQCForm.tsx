...
import { createIQCEntry, updateIQCEntry, getGRNItems } from '@/app/actions/quality'
import type { IQCEntry, IQCFormData, GRNSelectOption, GRNItem } from '@/types/quality'
...
export default function IQCForm({ entry, grns, products, users, currentUserId }: Props) {
  ...
  useEffect(() => {
    if (form.sample_size > 0) {
      ...
      } else if (rejRate > 0.1 || (!form.visual_check && !form.dimension_check)) {
        setForm(f => ({ ...f, result: 'Fail' }))
      } else if (rejRate > 0) {
        setForm(f => ({ ...f, result: 'Rework' }))
      }
    }
  }, [form.accepted_qty, form.rejected_qty, form.visual_check, form.dimension_check, form.sample_size])
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>{isEdit ? 'Edit IQC Entry' : 'New IQC Entry'}</h1>
          <p className={styles.subtitle}>Complete inspection details for incoming goods</p>
        </div>
        <button className={styles.ghostBtn} onClick={() => router.push('/dashboard/quality/iqc')}>
          ← Back to List
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className={styles.formCard}>
        ...
        <div className={styles.formSection}>
          ...
          <div className={styles.formGrid}>
            ...
            <div className={styles.field}>
              <label>Select GRN <span className={styles.req}>*</span></label>
              <select
                value={form.grn_id}
              ...
              >
                ...
                {grns.map(g => (
                  <option key={g.id} value={g.id}>
                    {g.grn_no} — {g.vendor_name} ({formatDate(g.gate_entry_date)})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className={styles.field}>
              ...
              {grnItems.length > 0 ? (
                ...
                <select
                  ...
                  required
                >
                  <option value={0} disabled>{loadingItems ? 'Loading…' : '— select product —'}</option>
                  {grnItems.map(i => (
                    <option key={i.product_id} value={i.product_id}>
                      {i.product_code} — {i.product_name} (received: {i.received_qty})
                    </option>
                  ))}
                </select>
              ) : (
            ...
            {selectedGRN && (
              <div className={styles.field}>
                <label>Vendor</label>
                <input value={selectedGRN.vendor_name} disabled className={styles.readOnly} />
              </div>
            )}
        ...
        <div className={styles.formSection}>
          ...
          <div className={styles.formGrid}>
            ...
            <div className={styles.field}>
              <label>Total Received Qty <span className={styles.req}>*</span></label>
              <input
                type="number" min={0}
            ...
            <div className={styles.field}>
              <label>Sample Size <span className={styles.req}>*</span></label>
              <input
                type="number" min={1} max={form.total_qty}
              ...
              {form.total_qty > 0 && (
                <span className={styles.fieldHint}>
                  {form.sample_size > 0 ? `${((form.sample_size / form.total_qty) * 100).toFixed(0)}% sample` : ''}
                </span>
              )}
            </div>

            <div className={styles.field}>
              <label>Accepted Qty <span className={styles.req}>*</span></label>
              <input
                type="number" min={0} max={form.sample_size}
            ...
            <div className={styles.field}>
              ...
              {form.rejected_qty > 0 && form.sample_size > 0 && (
                <span className={styles.fieldHintDanger}>
                  {((form.rejected_qty / form.sample_size) * 100).toFixed(1)}% rejection rate
                </span>
              )}
            </div>
        ...
        <div className={styles.formSection}>
          ...
          <div className={styles.checkGrid}>
            ...
            <label className={styles.checkboxCard}>
              ...
              />
              <div className={`${styles.checkContent} ${form.visual_check ? styles.checkedCard : ''}`}>
                <span className={styles.checkIcon}>👁</span>
                <div>
                  <p className={styles.checkLabel}>Visual Check</p>
                  <p className={styles.checkDesc}>Surface defects, colour, finish, labelling</p>
                </div>
                <span className={`${styles.checkStatus} ${form.visual_check ? styles.checkStatusPass : styles.checkStatusFail}`}>
            ...
            <label className={styles.checkboxCard}>
              ...
              />
              <div className={`${styles.checkContent} ${form.dimension_check ? styles.checkedCard : ''}`}>
                <span className={styles.checkIcon}>📐</span>
                <div>
                  <p className={styles.checkLabel}>Dimension Check</p>
                  <p className={styles.checkDesc}>Size, weight, tolerance within spec</p>
                </div>
                <span className={`${styles.checkStatus} ${form.dimension_check ? styles.checkStatusPass : styles.checkStatusFail}`}>
        ...
        <div className={styles.formSection}>
            ...
            {RESULTS.map(r => (
              <label key={r} className={`${styles.resultCard} ${form.result === r ? styles[`resultCard${r}`] : ''}`}>
                ...
                <span className={styles.resultCardDesc}>
                  {r === 'Pass'   ? 'Accept batch — move to stores'  :
                   r === 'Fail'   ? 'Reject batch — return to vendor' :
          ...
          <p className={styles.autoHint}>
            💡 Result is auto-suggested based on rejection rate and check outcomes. You can override it.
          </p>
        </div>
        ...
        <div className={styles.formFooter}>
          <button type="button" className={styles.cancelBtn} onClick={() => router.push('/dashboard/quality/iqc')}>
            Cancel
          </button>
