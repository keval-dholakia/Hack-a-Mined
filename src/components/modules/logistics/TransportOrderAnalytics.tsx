...
import { TransportOrderRecord, STATUS_COLOR, MODE_COLOR, SEED } from './TransportOrder'
...
function KPI({ icon, label, value, color }: { icon:string; label:string; value:string; color:string }) {
  return (
    <div className={toStyles.kpiCard}>
      <div className={toStyles.kpiIcon} style={{ color, background:`${color}18` }}>{icon}</div>
      <div><p className={toStyles.kpiLabel}>{label}</p><p className={toStyles.kpiValue} style={{ color }}>{value}</p></div>
    </div>
  )
}

interface Props { records: TransportOrderRecord[]; onClose: () => void }

export default function TransportOrderAnalytics({ records, onClose }: Props) {
  const [tab, setTab] = useState<'overview'|'routes'|'transporters'|'trend'>('overview')
  ...
  const stats = useMemo(() => {
    ...
    const totalWeight  = records.reduce((s,r)=>s+r.items.reduce((a,i)=>a+i.weight_kg,0),0)
    const totalPkgs    = records.reduce((s,r)=>s+r.items.reduce((a,i)=>a+i.packages,0),0)
    const avgFreight   = total?Math.round(totalFreight/total):0

    const statusPie = [
      {name:'Delivered',value:delivered},{name:'In Transit',value:inTransit},
      {name:'Dispatched',value:dispatched},{name:'Booked',value:booked},
    ...
    records.forEach(r=>{
      ...
      modeMap[r.mode].count++; modeMap[r.mode].freight+=r.freight_amt
    })
    const modeData = Object.entries(modeMap).map(([name,d])=>({name,...d}))
    ...
    const monthData = Array.from({length:12},(_,m)=>{
      ...
      return { label:lbl, total:mos.length, delivered:mos.filter(r=>r.status==='Delivered').length, freight:mos.reduce((s,r)=>s+r.freight_amt,0) }
    })

    // Transporter breakdown
    const tMap: Record<string,{count:number;delivered:number;freight:number}> = {}
    records.forEach(r=>{
      if(!tMap[r.transporter]) tMap[r.transporter]={count:0,delivered:0,freight:0}
    ...
    records.forEach(r=>{ if(!cMap[r.customer]) cMap[r.customer]={count:0,freight:0}; cMap[r.customer].count++; cMap[r.customer].freight+=r.freight_amt })
    const topCustomers = Object.entries(cMap).sort((a,b)=>b[1].count-a[1].count).slice(0,8).map(([name,d])=>({name,revenueB:name.split(' ')[0],...d}))
    const radarData    = topCustomers.slice(0,6).map(c=>({ cust:c.name.split(' ')[0], count:c.count, freightL:Math.round(c.freight/1_00_000) }))

    // Route popularity
    const routeMap: Record<string,number> = {}
    records.forEach(r=>{ const key=`${r.origin}→${r.destination}`; routeMap[key]=(routeMap[key]??0)+1 })
    const topRoutes = Object.entries(routeMap).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([name,value])=>({name,value}))

    return { total,delivered,inTransit,dispatched,booked,cancelled,draft,deliveryRate,totalFreight,totalWeight,totalPkgs,avgFreight,statusPie,modePie,modeData,monthData,transporterData,topCustomers,radarData,topRoutes }
  },[records])

  const tabs = [
    {id:'overview',label:'⊞ Overview'},{id:'routes',label:'◈ Routes & Modes'},
    {id:'transporters',label:'◉ Transporters'},{id:'trend',label:'◎ Trend & Health'},
  ] as const
    ...
    <div className={toStyles.analyticsPage}>
      <div className={toStyles.analyticsHeader}>
        <div>
          ...
          <p className={toStyles.analyticsSub}>360° logistics data visualization · {stats.total} orders</p>
        </div>
        <button className={toStyles.backBtn} onClick={onClose}>← Back to List</button>
      </div>
      ...
      <div className={toStyles.kpiStrip}>
        ...
        <KPI icon="▣" label="In Transit"      value={String(stats.inTransit)}         color={AMBER}   />
        <KPI icon="⬡" label="Delivery Rate"   value={`${stats.deliveryRate}%`}       color={CYAN}    />
        <KPI icon="◎" label="Total Freight"   value={fmtCur(stats.totalFreight)}      color={PURPLE}  />
        <KPI icon="◇" label="Total Weight"    value={`${stats.totalWeight} kg`}       color={ROSE}    />
      </div>

      <div className={toStyles.tabBar}>
        {tabs.map(t=>(
          <button key={t.id} className={`${toStyles.aTab} ${tab===t.id?toStyles.aTabActive:''}`} onClick={()=>setTab(t.id)}>{t.label}</button>
        ))}
      </div>
      ...
      {tab==='overview' && (
        <div className={toStyles.aGrid}>
          ...
          <div className={toStyles.aCard}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart><Pie data={stats.modePie} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={48} paddingAngle={4} label={({name,percent})=>`${name} ${((percent??0)*100).toFixed(0)}%`}>
                {stats.modePie.map((e,i)=><Cell key={i} fill={MODE_COLOR[e.name]??COLORS[i]}/>)}
              </Pie><Tooltip content={<DarkTooltip/>}/><Legend wrapperStyle={{color:'var(--text-muted)',fontSize:'0.78rem'}}/></PieChart>
            </ResponsiveContainer>
          </div>
          <div className={toStyles.aCard}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={stats.topRoutes} layout="vertical" barSize={18}>
                ...
                <Bar dataKey="value" name="Shipments" radius={[0,6,6,0]}>{stats.topRoutes.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className={toStyles.aCard}>
            ...
            <table className={toStyles.sTable}>
              ...
              <tbody>{stats.statusPie.map(row=>(
                <tr key={row.name}>
                  ...
                  <td className={toStyles.mono}>{stats.total?(row.value/stats.total*100).toFixed(1):0}%</td>
      ...
      {tab==='routes' && (
        <div className={toStyles.aGrid}>
          <div className={toStyles.aCard}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={stats.modeData} barSize={40}>
                ...
                <YAxis tick={{fill:'#6b7280',fontSize:11}} axisLine={false} tickLine={false} tickFormatter={v=>fmtCur(v)}/>
                <Tooltip content={<DarkTooltip formatter={fmtCur}/>}/>
                <Bar dataKey="freight" name="Freight" radius={[6,6,0,0]}>{stats.modeData.map((e,i)=><Cell key={i} fill={MODE_COLOR[e.name]??COLORS[i]}/>)}</Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className={toStyles.aCard}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <RadarChart data={stats.radarData} cx="50%" cy="50%" outerRadius={90}>
                ...
                <Radar name="Shipments" dataKey="count" stroke={ACCENT} fill={ACCENT} fillOpacity={0.15}/>
                <Radar name="Freight (L)" dataKey="freightL" stroke={GREEN} fill={GREEN} fillOpacity={0.15}/>
          ...
          <div className={toStyles.aCard} style={{gridColumn:'span 2'}}>
            ...
            <table className={toStyles.sTable}>
              <thead><tr><th>#</th><th>Customer</th><th>Shipments</th><th>Total Freight</th><th>Share</th></tr></thead>
              <tbody>{stats.topCustomers.map((c,i)=>(
                <tr key={c.name}>
                  ...
                  <td className={toStyles.mono} style={{color:GREEN}}>{fmtCur(c.freight)}</td>
                  <td><div style={{display:'flex',alignItems:'center',gap:8}}><div style={{height:6,width:`${stats.totalFreight?c.freight/stats.totalFreight*100:0}%`,maxWidth:80,background:COLORS[i%COLORS.length],borderRadius:4,minWidth:2}}/><span style={{fontSize:'0.75rem',color:'var(--text-muted)',fontFamily:'var(--mono)'}}>{stats.totalFreight?(c.freight/stats.totalFreight*100).toFixed(1):0}%</span></div></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
      ...
      {tab==='transporters' && (
        <div className={toStyles.aGrid}>
          <div className={toStyles.aCard}>
            <p className={toStyles.aCardTitle}>Transporter — Volume & Deliveries</p>
          ...
          <div className={toStyles.aCard}>
            <p className={toStyles.aCardTitle}>Transporter Leaderboard <span className={toStyles.aBadge}>by delivery rate</span></p>
            <table className={toStyles.sTable}>
              <thead><tr><th>#</th><th>Transporter</th><th>Total</th><th>Delivered</th><th>Freight</th><th>Rate</th></tr></thead>
              <tbody>{[...stats.transporterData].sort((a,b)=>b.rate-a.rate).map((t,i)=>(
                <tr key={t.fullName}>
                  <td style={{color:'#6b7280',fontFamily:'var(--mono)'}}>#{i+1}</td>
                  <td><strong>{t.fullName}</strong></td>
                  <td className={toStyles.mono}>{t.count}</td>
                  <td className={toStyles.mono} style={{color:GREEN}}>{t.delivered}</td>
                  <td className={toStyles.mono} style={{color:GREEN}}>{fmtCur(t.freight)}</td>
      ...
      {tab==='trend' && (
        <div className={toStyles.aGrid}>
          <div className={toStyles.aCard} style={{gridColumn:'span 2'}}>
            <p className={toStyles.aCardTitle}>12-Month Dispatch Trend <span className={toStyles.aBadge}>volume · delivered · freight</span></p>
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={stats.monthData}>
                ...
                <YAxis yAxisId="fr" orientation="right" tick={{fill:'#6b7280',fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>fmtCur(v)}/>
                <Tooltip content={<DarkTooltip/>}/>
                <Line yAxisId="vol" type="monotone" dataKey="total"     name="Dispatched" stroke={ACCENT} strokeWidth={2.5} dot={{fill:ACCENT,r:4}} activeDot={{r:6}}/>
                <Line yAxisId="vol" type="monotone" dataKey="delivered" name="Delivered"  stroke={GREEN}  strokeWidth={2}   dot={{fill:GREEN,r:3}}/>
                <Line yAxisId="fr"  type="monotone" dataKey="freight"   name="Freight"    stroke={AMBER}  strokeWidth={2}   dot={{fill:AMBER,r:3}} strokeDasharray="4 2"/>
                <Legend wrapperStyle={{color:'var(--text-muted)',fontSize:'0.78rem'}}/>
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div className={toStyles.aCard}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart><Pie data={[{name:'Delivered',value:stats.delivered},{name:'In Transit',value:stats.inTransit},{name:'Dispatched',value:stats.dispatched},{name:'Booked/Draft',value:stats.booked+stats.draft},{name:'Cancelled',value:stats.cancelled}].filter(d=>d.value>0)}
                ...
                <Cell fill={GREEN}/><Cell fill={AMBER}/><Cell fill={ACCENT}/><Cell fill={CYAN}/><Cell fill={ROSE}/>
              </Pie><Tooltip content={<DarkTooltip/>}/><Legend wrapperStyle={{color:'var(--text-muted)',fontSize:'0.78rem'}}/></PieChart>
            </ResponsiveContainer>
          </div>
          <div className={toStyles.aCard}>
            ...
            <div className={toStyles.healthPanel}>
              {[
                ...
                {label:'In Transit',    val:stats.total?Math.round(stats.inTransit/stats.total*100):0, color:AMBER},
                {label:'Cancellation',  val:stats.total?Math.round(stats.cancelled/stats.total*100):0, color:ROSE},
              ].map(row=>(
                <div key={row.label} className={toStyles.hRow}>
                  <span className={toStyles.hLabel}>{row.label}</span>
                  <div className={toStyles.hTrack}><div className={toStyles.hFill} style={{width:`${row.val}%`,background:row.color}}/></div>
                  <span className={toStyles.hPct} style={{color:row.color}}>{row.val}%</span>
                </div>
              ))}
              <div className={toStyles.insight}>
                <span>
                  {stats.deliveryRate>=70?'Excellent delivery performance — over 70% orders delivered.':stats.deliveryRate>=50?'Good flow. Track in-transit orders for timely delivery.':'High open orders. Review transporter SLAs and in-transit delays.'}
                  {' '}Total freight of <strong>{fmtCur(stats.totalFreight)}</strong> across <strong>{stats.total}</strong> orders, covering <strong>{stats.totalWeight} kg</strong>.
                </span>
              </div>
            </div>
          </div>
