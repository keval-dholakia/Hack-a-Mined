...
import { fmtDate } from './pdfConstants'

export async function downloadInvoiceHtmlPdf(inv: any) {
  // Create hidden container
  const id = '__invoice_pdf_temp'
  ...
  container.innerHTML = invoiceHtml(inv)
  ...
  const safeName = `Invoice_${inv.invoice_no || Date.now()}`.replace(/[^a-zA-Z0-9_\-]/g, '')
...
export default downloadInvoiceHtmlPdf
